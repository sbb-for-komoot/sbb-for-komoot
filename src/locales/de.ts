export default {
  // Journey form
  from: 'Von',
  fromPlaceholder: 'Abfahrtsbahnhof',
  to: 'Nach',
  hikeDeparture: 'Wanderstart',
  selectHike: 'Wanderung auswählen',
  dateOfTravel: 'Reisedatum',
  dep: 'ab',
  arr: 'an',

  // Date picker
  hour: 'Std',
  minute: 'Min',

  // Actions
  openKomoot: 'Komoot öffnen',
  close: 'Schliessen',
  aboutExtension: 'Über diese Erweiterung',
  language: 'Sprache',

  // Info popup
  infoTitle: 'Community-Projekt',
  infoBody1: 'Diese Erweiterung ist <strong>kein</strong> offizielles Produkt der <strong>SBB/CFF/FFS</strong> (Schweizerische Bundesbahnen) oder von <strong>Komoot</strong>.',
  infoBody2: 'Es ist ein unabhängiges, quelloffenes Community-Projekt, das diese beiden Systeme verbindet \u2014 damit Sie Ihre Bahnfahrt zum Start Ihrer nächsten Wanderung einfach planen können.',
  infoBody3: 'Diese Erweiterung läuft <strong>vollständig auf Ihrem Gerät</strong>. Es werden keine persönlichen Daten gesammelt oder an Dritte weitergegeben \u2014 die einzigen externen Anfragen gehen an <strong>SBB/CFF/FFS</strong> und <strong>Komoot</strong>, um Fahrplan- und Routeninformationen abzurufen.',
  infoBody4: 'Der Quellcode ist open-source und verfügbar auf <a href="https://github.com/sbb-for-komoot/sbb-for-komoot" target="_blank" rel="noopener">github.com/sbb-for-komoot/sbb-for-komoot</a>.',
  infoFooter: 'Nutzung auf eigenes Risiko. Überprüfen Sie Verbindungen stets auf der <a href="https://www.sbb.ch" target="_blank" rel="noopener">offiziellen SBB-Website</a>.',
  infoBugReport: 'Einen Fehler gefunden? <a href="https://github.com/sbb-for-komoot/sbb-for-komoot/issues" target="_blank" rel="noopener">Melden Sie ihn auf GitHub</a>.',

  // Connection list
  loading: 'Laden\u2026',
  noConnections: 'Keine Verbindungen gefunden',
  connectionHint: 'Abfahrtsort eingeben und Wanderung auswählen, um Verbindungen zu suchen',
  connectionsError: 'Verbindungen konnten nicht geladen werden. Bitte erneut versuchen.',
  earlierConnections: 'Frühere Verbindungen',
  laterConnections: 'Spätere Verbindungen',
  walk: 'Zu Fuss',
  checkSbb: 'Auf SBB anzeigen',

  // Connection card
  direction: 'Richtung {name}',
  platform: 'Gl. {num}',
  durationHM: '{h} Std. {m} Min.',
  durationM: '{m} Min.',

  // Route card
  routeNotFound: 'Route nicht gefunden',
  statDistance: 'Distanz',
  statDuration: 'Dauer',
  statElevationUp: '\u2191 Aufstieg',
  statElevationDown: '\u2193 Abstieg',
  statRating: 'Bewertung',
}
